...
const LiftyGame = () => {
  ...
  const drawFrontCanvas = useCallback(() => {
    ...
    if (gameState === GAME_STATES.MENU) {
      ...
      ctx.fillText('Version 7.0', cssW / 2, cssH - 100);
    }
  }, [gameState]);
...
// Audio system
  ...
  const updateUFOs = useCallback(() => {
    ...
    if (ufoSoundRef.current) {
      const hasActiveUFOs = gameData.current.ufos.length > 0;
      const shouldPlaySound = hasActiveUFOs && soundEnabled;
  ...
  const updateMeteors = useCallback(() => {
    ...
    // Play/stop meteor sound based on presence of meteors (similar to UFO logic)
    if (meteorSoundRef.current) {
      const hasActiveMeteors = gameData.current.meteors && gameData.current.meteors.length > 0;
      const isCurrentlyPlaying = !meteorSoundRef.current.paused;
      
      if (hasActiveMeteors && soundEnabled) {
        // Meteors present - play sound if not already playing
      ...
      } else if (!hasActiveMeteors || !soundEnabled) {
  ...
  const drawCowboyHatBalloons = useCallback((ctx) => {
    ...
    hatBalloons.forEach(balloon => {
      ...
      // Draw brown balloon (cowboy theme)
      ctx.fillStyle = '#8B4513';
      ctx.beginPath();
      ctx.ellipse(x, y - 55, 15, 18, 0, 0, Math.PI * 2);
      ctx.fill();
  ...
  const drawLevelBackground = (ctx) => {
    const currentLevel = gameData.current.level;
    // Cycle through available backgrounds (1-10), then repeat for levels 11-15
...
if (event.key === ' ' || event.key === 'ArrowUp') {
    ...
    // Dev hotkeys: 1-9 and 0 to jump to levels 1-10, q-u for levels 11-15
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      // Handle horizontal movement (smooth, time-based)
      {
        ...
        if (left && !right) {
          ...
          // (Only apply this when there's truly no input - not when keys are being pressed)
          if (gameData.current.onOutsidePlatform && !left && !right) {
            vx = 0;
            gameData.current.movementDirection = 0;
          }
        }
