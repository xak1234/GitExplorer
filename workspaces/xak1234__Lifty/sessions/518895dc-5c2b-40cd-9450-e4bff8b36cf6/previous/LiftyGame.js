...
const CANVAS_HEIGHT = 750; // Increased from 700
...
const ACCEL_X = 1600; // px/s^2
const FRICTION_GROUND = 1800; // px/s^2
...
const LiftyGame = () => {
  ...
  const [uiScale, setUiScaleInternal] = useState(() => {
    const saved = parseFloat(localStorage.getItem('liftyUiScale') || '1');
  ...
  const gameData = useRef({
    ...
    // Broken stickman on death
    showBrokenStickman: false, // whether to show broken stickman
    brokenStickmanTimer: 0, // frames to show broken stickman
    brokenStickmanX: 0, // X position of broken stickman
    brokenStickmanY: 0, // Y position of broken stickman
    // Parachute system
  ...
  const drawControlsCanvas = useCallback(() => {
    ...
    // Leave canvas fully transparent; no background fill
    // Draw control buttons - centered arcade-style cluster (shrunk)
    const leftRadius = 28; // Reduced from 36
    const rightRadius = 28; // Reduced from 36
    const jumpRadius = 38; // Reduced from 48
    const spacing = 18; // Reduced from 24
    ...
    const drawArcadeButton = (x, y, radius, baseColor, label) => {
      ...
      ctx.font = 'bold 12px "Press Start 2P", monospace'; // Reduced from 16px
  ...
  const drawFrontCanvas = useCallback(() => {
    ...
    // Cover the canvas with front.png (object-fit: cover behavior)
    ...
    if (gameState === GAME_STATES.MENU) {
      ...
      ctx.fillText('Version 5.0', cssW / 2, cssH - 100);
    }
  }, [gameState]);
  ...
  const drawDialCanvas = useCallback(() => {
    ...
    SCALE_OPTIONS.forEach((scale, index) => {
      ...
      // Labels
      const labelX = centerX + Math.cos(rad) * (radius + 13); // Reduced from 20
      const labelY = centerY + Math.sin(rad) * (radius + 13);
      ctx.fillStyle = '#aaa';
      ctx.font = '6px "Press Start 2P", monospace'; // Reduced from 8px
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(SCALE_LABELS[index], labelX, labelY);
    });
  ...
  const initializeFruitPlacements = useCallback((level) => {
    ...
    // Default for any higher levels
    const config = fruitConfig[level] || { apples: 12, bananas: 10 };
    ...
    let appleCount = 0;
    let currentRow = 5;
    while (appleCount < config.apples) {
    ...
    let bananaCount = 0;
    currentRow = 10;
    while (bananaCount < config.bananas) {
  ...
  const initializeBackgroundImages = useCallback(() => {
    const imagesToLoad = ['lift1.jpg', 'lift2.jpg', 'lift3.jpg', 'lift4.jpg', 'lift5.jpg'];
  ...
  const randomizeSideLiftSpeeds = useCallback(() => {
    ...
    console.log(`🎲 Randomized lift speeds: Left=${gameData.current.leftLiftSpeed.toFixed(2)}, Center=${gameData.current.currentLiftSpeed.toFixed(2)}, Right=${gameData.current.rightLiftSpeed.toFixed(2)}`);
  }, []);
// Audio system
  ...
  const drawCrushingCeiling = (ctx, shaftX, ceilingY) => {
    ...
    // Draw crushing spikes (pointing down)
    ctx.fillStyle = COLORS.RED;
    const spikeWidth = 8;
  ...
  }, [initializeShaftMarkers, randomizeSideLiftSpeeds]);
  ...
  // Draw broken stickman on death (before explosion)
  const drawBrokenStickman = (ctx, x, y) => {
    ctx.save();
    const color = gameData.current.stickmanColor || 'white';
    ...
    // Head (cracked, broken)
    ctx.beginPath();
    ctx.arc(x + PLAYER_WIDTH/2, y + 10, 6, 0, Math.PI * 2);
    ctx.stroke();
    
    // Crack in head
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2 - 3, y + 10);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 3, y + 10);
    ctx.stroke();
    
    // X eyes (dead)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2 - 3, y + 8);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 1, y + 10);
    ctx.moveTo(x + PLAYER_WIDTH/2 - 1, y + 8);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 3, y + 10);
    ctx.stroke();
    ...
    // Body (broken, bent)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2, y + 16);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 2, y + 30);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 2, y + 45);
    ctx.stroke();
    
    // Left arm (hanging loosely)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2, y + 22);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 8, y + 28);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 10, y + 35);
    ctx.stroke();
    
    // Right arm (broken angle)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2, y + 22);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 8, y + 30);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 6, y + 38);
    ctx.stroke();
    
    // Left leg (bent unnaturally)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2, y + 45);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 6, y + 58);
    ctx.lineTo(x + PLAYER_WIDTH/2 - 4, y + 70);
    ctx.stroke();
    
    // Right leg (collapsed)
    ctx.beginPath();
    ctx.moveTo(x + PLAYER_WIDTH/2, y + 45);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 4, y + 60);
    ctx.lineTo(x + PLAYER_WIDTH/2 + 6, y + 70);
    ctx.stroke();
  ...
  const drawPlayer = (ctx, x, y, isDead = false) => {
    if (isDead) {
      ...
      // Prepare cache key based on pose, direction, and color
      const dir = (() => {
        if (gameData.current.isJumping) {
      ...
      // Compute orientation and walk phase
      const dir = (() => {
      ...
      let armSwing = Math.sin(phase) * 0.6; // radians (walk)
      let legSwing = Math.sin(phase) * 0.5; // radians (walk)
    ...
    if (gameData.current.parachuteActive && !isDead) {
      ctx.save();
  ...
  const createGoreEffect = () => {
    ...
    // Show broken stickman briefly before explosion
    gameData.current.showBrokenStickman = true;
    gameData.current.brokenStickmanTimer = 45; // 45 frames (~0.75 seconds at 60fps)
    gameData.current.brokenStickmanX = playerX;
    gameData.current.brokenStickmanY = playerY;
  ...
  const updateMedpacks = useCallback(() => {
    ...
    medpacks.forEach(pack => {
      ...
      const speed = pack.speed || 0.8; // Fallback to 0.8 if speed not set
      pack.y += speed;
    ...
    if (gameData.current.medpackSpawnTimer > 0) {
      ...
      // Randomize falling speed between 0.4 and 1.2 (slower to faster than original)
      const fallSpeed = 0.4 + Math.random() * 0.8;
  ...
  const drawLevelBackground = (ctx) => {
    ...
    // Cycle through available backgrounds (1-10), then repeat
    const backgroundLevel = ((currentLevel - 1) % 10) + 1;
  ...
  }, [gameState, getGroundPlayerYByIndex, playSound]);
  // Handle keyboard input
  const handleKeyDown = useCallback((event) => {
    gameData.current.keys[event.key] = true;
...
if (event.key === ' ' || event.key === 'ArrowUp') {
    ...
    // Dev hotkeys: 1-9 and 0 to jump to levels 1-10
    if (gameState === GAME_STATES.PLAYING && ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].includes(event.key)) {
      const targetLevel = event.key === '0' ? 10 : parseInt(event.key);
      ...
      // Apply level configuration for target level
  ...
  }, [gameState, handleJump, initializeShaftMarkers, randomizeSideLiftSpeeds, saveSnapshot, loadSnapshot]);


  const handleKeyUp = useCallback((event) => {
    gameData.current.keys[event.key] = false;
  }, []);
...
const handleControlsTouch = useCallback(async (event) => {
    ...
    // New clustered control layout (mirror drawControlsCanvas)
    const leftRadius = 28; // Reduced from 36
    const rightRadius = 28; // Reduced from 36
    const jumpRadius = 38; // Reduced from 48
    const spacing = 18; // Reduced from 24
    ...
    // Calculate distances from touch point to button centers
    ...
    if (distToLeft <= leftRadius) {
      ...
      if (gameState === GAME_STATES.PLAYING) {
        ...
        // Initialize new game...
        gameData.current.score = 0;
        // Randomize platform spawn phases and densities for variety each new game
        gameData.current.platformSpawnPhases = [
          Math.floor(Math.random() * 2),
          Math.floor(Math.random() * 2),
          Math.floor(Math.random() * 2),
        ...
        // Apply level 1 configuration for debug mode
        applyLevelConfig(gameData.current, 1);
        initializeFruitPlacements(1);
        gameData.current.liftY = -LIFT_HEIGHT;
        gameData.current.leftLiftY = -LIFT_HEIGHT;
  ...
  // Handle touch end for controls canvas
  ...
  // Handle mouse controls for desktop (mousedown)
  const handleControlsMouseDown = useCallback(async (event) => {
    ...
    // Control layout (mirror drawControlsCanvas)
    const leftRadius = 28; // Reduced from 36
    const rightRadius = 28; // Reduced from 36
    const jumpRadius = 38; // Reduced from 48
    const spacing = 18; // Reduced from 24
    ...
    // Calculate distances from mouse point to button centers
    const distToLeft = Math.sqrt((canvasX - leftButtonX) ** 2 + (canvasY - buttonY) ** 2);
    const distToRight = Math.sqrt((canvasX - rightButtonX) ** 2 + (canvasY - buttonY) ** 2);
    const distToJump = Math.sqrt((canvasX - jumpButtonX) ** 2 + (canvasY - buttonY) ** 2);
  ...
  // Handle mouse up for controls canvas
...
const handleTouch = useCallback(async (event) => {
  ...
  const getAllWalls = () => {
    ...
    lifts.forEach(lift => {
      ...
      // Check if doors are open on each side
      const currentIdx = gameData.current.currentShaftIndex || 0;
      const leftDoorOpen = (
        (lift.idx === 0 && gameData.current.centerDoorLeftOpen) ||
        (lift.idx === -1 && gameData.current.leftLiftDoorLeftOpen) ||
        (lift.idx === 1 && gameData.current.rightLiftDoorLeftOpen)
      );
      const rightDoorOpen = (
        (lift.idx === 0 && gameData.current.centerDoorRightOpen) ||
        (lift.idx === -1 && gameData.current.leftLiftDoorRightOpen) ||
        (lift.idx === 1 && gameData.current.rightLiftDoorRightOpen)
      );
      
      // Left wall (if door is closed)
      if (!leftDoorOpen) {
        walls.push({
          type: 'left_wall',
          x: liftX + 10,
  ...
  const checkWallCollision = (newPlayerX) => {
    ...
    for (const wall of walls) {
      // Only check walls for the current lift
      if (wall.liftIndex !== currentIdx) continue;
  ...
  const gameLoop = useCallback((currentTime) => {
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      if (shouldMove) {
        ...
        // Update center lift position using current dynamic speed and reverse at bounds
        {
          ...
          if (gameData.current.level === 2 && gameData.current.liftY + LIFT_HEIGHT >= CANVAS_HEIGHT && !gameData.current.crushingCeilingActive) {
            ...
            console.log('Level2: Lift hit ground, activating crushing ceiling');
        ...
        if (gameData.current.level === 2 && gameData.current.leftCrushingCeilingActive) {
          ...
          if (currentIdx === leftIdx) {
            ...
            if (insideHoriz && crushedBySpikes) {
              console.log('Level2: Player hit left ceiling spikes!');
              playSound('death');
              createGoreEffect();
              gameData.current.lives--;
              if (gameData.current.lives <= 0) {
        ...
        if (sideLiftWin && !gameData.current.survivedImpact) {
          ...
          if (!allApplesCollected || !allBananasCollected) {
            // Can't advance yet - need to collect all fruits!
            console.log('⚠️ Cannot complete level yet! Need to collect all fruits first!');
            ...
            // Don't advance level - player must collect all fruits
            return;
          }
        ...
        // Update left lift with directional movement and reverse at bounds
        {
          ...
          if (gameData.current.level === 2 && gameData.current.leftLiftY + LIFT_HEIGHT >= CANVAS_HEIGHT && !gameData.current.leftCrushingCeilingActive) {
            ...
            gameData.current.leftCrushingCeilingY = gameData.current.leftLiftY;
            gameData.current.leftCrushingCeilingDir = 1;
            console.log('Level2: Left lift hit ground, activating crushing ceiling');
        ...
        // Update right lift with directional movement and reverse at bounds
        {
          ...
          if (gameData.current.level === 2 && gameData.current.rightLiftY + LIFT_HEIGHT >= CANVAS_HEIGHT && !gameData.current.rightCrushingCeilingActive) {
            ...
            gameData.current.rightCrushingCeilingY = gameData.current.rightLiftY;
            gameData.current.rightCrushingCeilingDir = 1;
            console.log('Level2: Right lift hit ground, activating crushing ceiling');
      ...
      // Wrap rows that move past the top/bottom to keep platforms continuous
      {
        ...
        gameData.current.shaftLines.forEach((line, idxLine) => {
          ...
          if (line.y < -rowSpacing) {
            ...
            // If player is riding this platform line, move them by the same delta so they reappear at the opposite side
            if (gameData.current.onOutsidePlatform && gameData.current.ridingPlatformLineIndex === idxLine) {
              const dy = line.y - oldY;
              gameData.current.playerY += dy;
              // Also update fall baseline to prevent false big-fall death
              gameData.current.jumpStartFeetY += dy;
              gameData.current.bigFallTriggered = false;
              console.log(`Platform wrapped UP (top to bottom): lineIndex=${idxLine}, oldY=${oldY.toFixed(1)}, newY=${line.y.toFixed(1)}, dy=${dy.toFixed(1)}, playerY now=${gameData.current.playerY.toFixed(1)}`);
            }
          }
        ...
        const furthestBottom = Math.max(...gameData.current.shaftLines.map(line => line.y));
        
        if (furthestBottom < bufferDistance) {
          const newPlatforms = [];
          let currentY = furthestBottom + rowSpacing;
          
          // Add platforms until we have enough buffer
          while (currentY < bufferDistance) {
            newPlatforms.push({
              y: currentY,
      ...
      // Handle horizontal movement (smooth, time-based)
      {
        ...
        if (left && !right) {
          ...
          // If riding a platform and no input, stay put horizontally on it
      ...
      // Update animation (inspired by Konva.js frame-based animation techniques)
      if (gameData.current.movementDirection !== 0) {
        // Animate when moving - use frame-based timing for smooth 60fps animation
        gameData.current.animationTimer++;
        // Change frame every 10 ticks for 6fps animation (60fps / 10 = 6fps) - slower, smoother
        if (gameData.current.animationTimer >= 10) { 
      ...
      // PHYSICS: Check for wall collisions BEFORE applying horizontal movement
      {
        ...
        if (wallCheck.collision) {
          // Hit a wall - stop movement completely (no position adjustment to prevent shaking)
          if (wallCheck.type === 'left') {
            // Moving left into left wall - stop all leftward velocity
            if (gameData.current.playerVelocityX < 0) {
              gameData.current.playerVelocityX = 0;
            }
            if (gameData.current.isJumping && gameData.current.playerVelocityX < 0) {
              gameData.current.playerVelocityX = Math.abs(gameData.current.playerVelocityX) * 0.3;
            }
          ...
          // No wall collision - apply movement normally
          gameData.current.playerX = proposedNewX;
        }
      }
      ...
      // Check if all cherries collected for the current level -> auto-complete level
      if (gameData.current.level === 1) {
        const allCherriesCollected = gameData.current.leftKeyCollected && gameData.current.rightKeyCollected;
        const allApplesCollected = gameData.current.applesCollected >= gameData.current.requiredApplesPerLevel;
        const allBananasCollected = gameData.current.bananasCollected >= gameData.current.requiredBananasPerLevel;
      ...
      // Check if all cherries collected for Level 3 -> auto-complete level
      if (gameData.current.level === 3) {
        const allCherriesCollected = gameData.current.leftKeyCollected && gameData.current.centerKeyCollected && gameData.current.rightKeyCollected;
        const allApplesCollected = gameData.current.applesCollected >= gameData.current.requiredApplesPerLevel;
        const allBananasCollected = gameData.current.bananasCollected >= gameData.current.requiredBananasPerLevel;
        ...
        if (allCherriesCollected && !gameData.current.survivedImpact) {
          // Check if all required fruits have been collected
          if (!allApplesCollected || !allBananasCollected) {
            console.log('⚠️ Cherries collected, but still need fruits to complete Level 3!');
            console.log(`🍏 Apples: ${gameData.current.applesCollected}/${gameData.current.requiredApplesPerLevel}`);
            console.log(`🍌 Bananas: ${gameData.current.bananasCollected}/${gameData.current.requiredBananasPerLevel}`);
            // Don't advance level yet
          } else {
            console.log('All cherries collected in Level 3! Level completed!');
            gameData.current.survivedImpact = true;
            playSound('levelUp');
            gameData.current.score += gameData.current.level * 100;
          ...
          if (gameData.current.level >= 3) {
            ...
            initializeShaftMarkers();
            logPlatformConfig();
            console.log('DEBUG: All cherries collected in Level 3 -> level advanced (wrap mode)');
          } else {
            gameData.current.level++;
            console.log('All cherries collected in Level 3! Level completed! New level:', gameData.current.level);
            ...
            setTimeout(() => {
              ...
              initializeShaftMarkers();
              logPlatformConfig();
              console.log('DEBUG: All cherries collected in Level 3 -> next level started');
            }, 1000);
          }
          }
        }
      ...
      // Gentle bounds checking for extreme off-screen positions only
      const extremeLimit = CANVAS_WIDTH; // Very generous limit
      if (gameData.current.playerX < -extremeLimit) {
        gameData.current.playerX = -extremeLimit;
        gameData.current.playerVelocityX = 0;
      } else if (gameData.current.playerX > extremeLimit) {
        gameData.current.playerX = extremeLimit;
        gameData.current.playerVelocityX = 0;
      }
      ...
      if (gameData.current.isJumping) {
        // Apply gravity with parachute modification
        let gravity = GRAVITY;
        let terminalVelocity = TERMINAL_VELOCITY;
        
        if (gameData.current.parachuteActive) {
          // Parachute reduces gravity and terminal velocity significantly
          gravity = GRAVITY * 0.3; // Much slower fall acceleration
          terminalVelocity = TERMINAL_VELOCITY * 0.4; // Much slower max fall speed
        }
        
        gameData.current.jumpVelocity = Math.min(terminalVelocity, gameData.current.jumpVelocity + gravity);
        gameData.current.playerY += gameData.current.jumpVelocity;
        ...
        // Mid-air big-fall instant death if exceeded distance and still no support (unless parachute is active)
        {
          const feet = gameData.current.playerY + PLAYER_HEIGHT;
          const startFeet = (typeof gameData.current.jumpStartFeetY === 'number') ? gameData.current.jumpStartFeetY : feet;
          if (!gameData.current.bigFallTriggered && !gameData.current.parachuteActive && (feet - startFeet >= BIG_FALL_DEATH_HEIGHT) && (gameData.current.airTicks || 0) >= 25) {
            ...
            if (gameData.current.lives <= 0) {
              ...
              setTimeout(() => {
                // Restart level after big-fall death
                gameData.current.liftY = -LIFT_HEIGHT;
                gameData.current.leftLiftY = -LIFT_HEIGHT;
                gameData.current.rightLiftY = -LIFT_HEIGHT;
                gameData.current.centerLiftDir = 1;
        ...
        if (!gameData.current.bigFallTriggered) {
          ...
          for (const surface of surfaces) {
            ...
            if (hasSignificantOverlap && (crossingDown || nearSurface) && gameData.current.jumpVelocity > -0.5) {
              // Check for big fall death (unless parachute is active)
              ...
              if (!gameData.current.parachuteActive && (feetAtLanding - startFeet >= BIG_FALL_DEATH_HEIGHT) && (gameData.current.airTicks || 0) >= 25) {
                // Fatal fall
                ...
                gameData.current.isJumping = false;
                // Reset parachute and jump state on landing
                if (gameData.current.parachuteActive) {
                  gameData.current.parachuteActive = false;
                  console.log('🪂 Parachute auto-removed on landing');
                }
        ...
        // If still falling and passed bottom, die (fell into abyss)
        if (gameData.current.isJumping && (gameData.current.playerY + PLAYER_HEIGHT >= CANVAS_HEIGHT)) {
      ...
      // Level 1: cherries in left and right lifts
      if (gameData.current.level === 1) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyRight(ctx);
      }
      // Level 2: cherries in all lifts (left, center, right)
      if (gameData.current.level === 2) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyCenter(ctx);
        drawGoldenKeyRight(ctx);
      }
      // Level 3: cherries in all lifts (left, center, right)
      if (gameData.current.level === 3) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyCenter(ctx);
        drawGoldenKeyRight(ctx);
      ...
      // Handle broken stickman display with pause
      if (gameData.current.showBrokenStickman && gameData.current.brokenStickmanTimer > 0) {
        // Show broken stickman with pause
        drawBrokenStickman(ctx, gameData.current.brokenStickmanX, gameData.current.brokenStickmanY);
        gameData.current.brokenStickmanTimer--;
        
        // Skip rest of game logic while showing broken stickman (pause)
        if (gameData.current.brokenStickmanTimer <= 0) {
          gameData.current.showBrokenStickman = false;
        }
      } else if (Date.now() < gameData.current.deathEffectUntil) {
        // Show death gore effects after broken stickman
        updateBodyParts();
        drawPlayer(ctx, playerX, playerY, true);
    ...
    if (gameState === GAME_STATES.DEAD) {
      ...
      // Level 1: cherries in left and right lifts (also visible on death screen)
      if (gameData.current.level === 1) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyRight(ctx);
      }
      // Level 2: cherries in all lifts (also visible on death screen)
      if (gameData.current.level === 2) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyCenter(ctx);
        drawGoldenKeyRight(ctx);
      }
      // Level 3: cherries in all lifts (also visible on death screen)
      if (gameData.current.level === 3) {
        drawGoldenKeyLeft(ctx);
        drawGoldenKeyCenter(ctx);
        drawGoldenKeyRight(ctx);
      ...
      // Update and draw gore
      updateBodyParts();
      const currentIdxDead = gameData.current.currentShaftIndex || 0;
      const shaftXDead = getShaftXByIndex(currentIdxDead);
      const liftCenterX = shaftXDead + (SHAFT_WIDTH - PLAYER_WIDTH) / 2;
      const playerX = liftCenterX + gameData.current.playerX; // Add horizontal offset
      
      // Use the position where player died
      let playerY;
      if (gameData.current.isJumping) {
        playerY = gameData.current.playerY;
      } else {
        const groundY = getGroundPlayerYByIndex(currentIdxDead);
        playerY = groundY;
      }
  ...
  }, [gameState, drawPlatforms, drawLift, drawLiftCable, drawLevelBackground, calculateDynamicSpeed, handleJump, drawStatsCanvas, drawFrontCanvas, drawTitleCanvas, updateBalloonDogs, drawBalloonDogs, updateMedpacks, drawMedpacks, checkMedpackCollision, updateDynamiteBalloons, drawDynamiteBalloons, checkDynamiteBalloonCollision]);
...
useEffect(() => {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ position: 'relative', display: 'inline-block', transform: `scale(${uiScale})`, transformOrigin: 'top center' }}>
      ...
      <canvas
        ...
        style={{
          imageRendering: 'pixelated',
          display: 'block',
          margin: '-20px auto 0 auto', // Move upward with negative top margin
          width: '100%',
          maxWidth: '680px',
          height: 'auto'
        }}
      ...
      <canvas
        ...
        style={{
          imageRendering: 'pixelated',
          display: 'block',
          margin: '0 auto 0 auto', // No top margin
