...
const LiftyGame = () => {
  ...
  const drawFrontCanvas = useCallback(() => {
    ...
    if (gameState === GAME_STATES.MENU) {
      ...
      ctx.fillText('Version 6.0', cssW / 2, cssH - 100);
    }
  }, [gameState]);
  ...
  const initializeBackgroundImages = useCallback(() => {
    const imagesToLoad = ['lift1.jpg', 'lift2.jpg', 'lift3.jpg', 'lift4.jpg', 'lift5.jpg', 'lift6.jpg', 'lift7.jpg', 'lift8.jpg', 'lift9.jpg', 'lift10.jpg'];
  ...
  const handleStatsClick = useCallback((event) => {
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      if (x >= arrowX && x <= arrowX + arrowSize && y >= arrowY && y <= arrowY + arrowSize) {
        // Advance to next level (wrap from 10 to 1)
...
// Audio system
  ...
  const updateUFOs = useCallback(() => {
    ...
    // Check if player is currently in a death state - prevent multiple deaths
    const now = Date.now();
    ...
    if (!isInDeathState) {
      gameData.current.ufos.forEach(ufo => {
        ...
        if (beamHeight > 0 && playerBounds.y + playerBounds.height > beamTopY) {
          const playerCenterY = playerBounds.y + playerBounds.height / 2;
          const relativeY = playerCenterY - beamTopY;
          const beamWidthAtPlayer = beamTopWidth + (beamBottomWidth - beamTopWidth) * (relativeY / beamHeight);
          
          const beamLeft = ufo.x - beamWidthAtPlayer / 2;
          const beamRight = ufo.x + beamWidthAtPlayer / 2;
          const playerCenterX = playerBounds.x + playerBounds.width / 2;
          
          // Check if player center is within beam
          if (playerCenterX > beamLeft && playerCenterX < beamRight && 
              playerCenterY > beamTopY && playerCenterY < beamBottomY) {
    ...
    gameData.current.ufos = gameData.current.ufos.filter(ufo => 
      ufo && ufo.x > -100 && ufo.x < CANVAS_WIDTH + 100
    );
    
    // Play/stop UFO sound based on presence of UFOs
    if (ufoSoundRef.current) {
      if (gameData.current.ufos.length > 0 && soundEnabled) {
        // UFOs present - play sound if not already playing
        if (ufoSoundRef.current.paused) {
          try {
            if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
              audioContextRef.current.resume();
            }
            ufoSoundRef.current.play().then(() => {
              console.log('UFO sound playing');
            }).catch(err => {
...
if (event.key === ' ' || event.key === 'ArrowUp') {
  ...
  }, [gameState, handleJump, initializeShaftMarkers, randomizeSideLiftSpeeds, saveSnapshot, loadSnapshot, initializeFruitPlacements, logPlatformConfig, calculateDynamicSpeed, playLevelSound]);
  ...
  const gameLoop = useCallback((currentTime) => {
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      if (gameData.current.isJumping) {
        ...
        // If still falling and passed bottom, die (fell into abyss) - but not in wrap mode
        if (!gameData.current.wrapModeEnabled && !gameData.current.bigFallTriggered && gameData.current.isJumping && (gameData.current.playerY + PLAYER_HEIGHT >= CANVAS_HEIGHT)) {
          // Check if fallen far enough to die
          const feetAtBottom = CANVAS_HEIGHT;
          const startFeet = (typeof gameData.current.jumpStartFeetY === 'number') ? gameData.current.jumpStartFeetY : feetAtBottom;
          const fellFarEnough = !gameData.current.parachuteActive && (feetAtBottom - startFeet >= BIG_FALL_DEATH_HEIGHT) && (gameData.current.airTicks || 0) >= 25;
          ...
          if (fellFarEnough) {
            ...
            // Not a fatal fall - just clamp to bottom and land safely
            gameData.current.playerY = CANVAS_HEIGHT - PLAYER_HEIGHT;
            gameData.current.isJumping = false;
            gameData.current.jumpVelocity = 0;
            gameData.current.parachuteActive = false;
            console.log('🎮 PHYSICS: Landed safely at bottom (fall not far enough for death)');
          }
        }
      }
