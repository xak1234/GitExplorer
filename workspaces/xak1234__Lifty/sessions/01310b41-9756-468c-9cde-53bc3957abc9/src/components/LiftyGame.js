...
const LiftyGame = () => {
  ...
  const lifthitSoundRef = useRef(null); // For meteor hitting lift sound
  const liftfallSoundRef = useRef(null); // For lift falling/crashing sound
  ...
  const gameData = useRef({
    ...
    // Lift states for meteor collision system
    liftStates: {
      left: { status: 'normal', fallVelocity: 0, crashedY: null }, // status: 'normal', 'falling', 'crashed'
      center: { status: 'normal', fallVelocity: 0, crashedY: null },
      right: { status: 'normal', fallVelocity: 0, crashedY: null }
    },
  ...
  const drawFrontCanvas = useCallback(() => {
    ...
    if (gameState === GAME_STATES.MENU) {
      ...
      ctx.fillText('Version 9.0', cssW / 2, cssH - 100);
  ...
  // Initialize lifthit sound for meteor hitting lift
  const initializeLifthitSound = useCallback(() => {
    const audio = new Audio();
    
    audio.oncanplaythrough = () => {
      console.log('Lifthit sound loaded successfully!');
      lifthitSoundRef.current = audio;
    };
    
    audio.onloadeddata = () => {
      console.log('Lifthit sound data loaded');
      if (!lifthitSoundRef.current) {
        lifthitSoundRef.current = audio;
      }
    };
    
    audio.onerror = () => {
      console.error('Failed to load lifthit sound from sound/lifthit.mp3');
    };
    
    audio.src = 'sound/lifthit.mp3';
    audio.preload = 'auto';
    audio.volume = 0.8;
    audio.load();
  }, []);

  // Initialize liftfall sound for lift crashing
  const initializeLiftfallSound = useCallback(() => {
    const audio = new Audio();
    
    audio.oncanplaythrough = () => {
      console.log('Liftfall sound loaded successfully!');
      liftfallSoundRef.current = audio;
    };
    
    audio.onloadeddata = () => {
      console.log('Liftfall sound data loaded');
      if (!liftfallSoundRef.current) {
        liftfallSoundRef.current = audio;
      }
    };
    
    audio.onerror = () => {
      console.error('Failed to load liftfall sound from sound/liftfall.mp3');
    };
    
    audio.src = 'sound/liftfall.mp3';
    audio.preload = 'auto';
    audio.volume = 0.8;
    audio.load();
  }, []);
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      if (x >= arrowX && x <= arrowX + arrowSize && y >= arrowY && y <= arrowY + arrowSize) {
        ...
        try {
          ...
          // Reset lift states (meteor collision system)
          gameData.current.liftStates = {
            left: { status: 'normal', fallVelocity: 0, crashedY: null },
            center: { status: 'normal', fallVelocity: 0, crashedY: null },
            right: { status: 'normal', fallVelocity: 0, crashedY: null }
          };
  ...
  // Play lifthit sound when meteor hits lift
  const playLifthitSound = useCallback(() => {
    if (lifthitSoundRef.current && soundEnabled) {
      try {
        if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
          audioContextRef.current.resume();
        }
        
        lifthitSoundRef.current.currentTime = 0;
        const playPromise = lifthitSoundRef.current.play();
        if (playPromise !== undefined) {
          playPromise.then(() => {
            console.log('Lifthit sound played successfully');
          }).catch(error => {
            console.error('Error playing lifthit sound:', error);
          });
        }
      } catch (error) {
        console.error('Error playing lifthit sound:', error);
  ...
  // Play liftfall sound when lift crashes to ground
  const playLiftfallSound = useCallback(() => {
    if (liftfallSoundRef.current && soundEnabled) {
      try {
        if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
          audioContextRef.current.resume();
        }
        
        liftfallSoundRef.current.currentTime = 0;
        const playPromise = liftfallSoundRef.current.play();
        if (playPromise !== undefined) {
          playPromise.then(() => {
            console.log('Liftfall sound played successfully');
          }).catch(error => {
            console.error('Error playing liftfall sound:', error);
          });
        }
      } catch (error) {
        console.error('Error playing liftfall sound:', error);
...
// Audio system
  ...
  const resetLevelAfterDeath = useCallback(() => {
    ...
    gameData.current.currentShaftIndex = 0;
    // DON'T reset cherry collection states on respawn - cherries should stay collected
  ...
  const drawLiftCable = (ctx) => {
    ...
    const liftX = shaftX + (SHAFT_WIDTH - LIFT_WIDTH) / 2;
    const liftY = gameData.current.liftY;
    const liftState = gameData.current.liftStates.center;
    ...
    if (liftState.status === 'crashed') {
      // Draw broken cable - only show cable from top to break point
      const breakY = Math.min(200, liftY - 50); // Cable breaks above the lift
      
      // Draw cable from top to break point
      ctx.strokeStyle = COLORS.LIGHT_GRAY;
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(cableX, cableEndY);
      ctx.lineTo(cableX, breakY);
      ctx.stroke();

      // Draw cable segments up to break point
      ctx.strokeStyle = COLORS.DARK_GRAY;
      ctx.lineWidth = 1;
      const segmentLength = 20;
      for (let y = cableEndY; y < breakY; y += segmentLength) {
        ctx.beginPath();
      ...
      // Draw frayed cable end at break point
      ctx.strokeStyle = COLORS.DARK_GRAY;
      ctx.lineWidth = 2;
      for (let i = 0; i < 5; i++) {
        const frayX = cableX + (Math.random() - 0.5) * 6;
        const frayLength = 5 + Math.random() * 10;
        ctx.beginPath();
        ctx.moveTo(frayX, breakY);
        ctx.lineTo(frayX + (Math.random() - 0.5) * 4, breakY + frayLength);
        ctx.stroke();
      }
      ...
      // No attachment point on lift since cable is broken
    } else {
      // Draw normal intact cable
      ctx.strokeStyle = COLORS.LIGHT_GRAY;
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(cableX, cableEndY);
    ...
    meteors.forEach((meteor, meteorIdx) => {
      ...
      if (!gameData.current.invincible) {
        ...
        if (dx < hitRadius && dy < hitRadius) {
          ...
          // Lose a life
          gameData.current.lives--;
          console.log(`Player hit by meteor! Lives remaining: ${gameData.current.lives}`);
      ...
      // Check collision with lifts
      const liftPositions = [
        { index: -1, x: getShaftXByIndex(-1), y: gameData.current.leftLiftY, key: 'left' },
        { index: 0, x: getShaftXByIndex(0), y: gameData.current.liftY, key: 'center' },
        { index: 1, x: getShaftXByIndex(1), y: gameData.current.rightLiftY, key: 'right' }
      ];

      liftPositions.forEach(lift => {
        const liftState = gameData.current.liftStates[lift.key];
        if (liftState.status === 'crashed') return; // Skip already crashed lifts
        
        const liftX = lift.x + (SHAFT_WIDTH - LIFT_WIDTH) / 2;
        const liftCenterX = liftX + LIFT_WIDTH / 2;
        const liftCenterY = lift.y + LIFT_HEIGHT / 2;
        
        const dx = Math.abs(meteor.x - liftCenterX);
        const dy = Math.abs(meteor.y - liftCenterY);
        const hitRadius = meteor.size + Math.max(LIFT_WIDTH, LIFT_HEIGHT) / 2;

        if (dx < hitRadius && dy < hitRadius) {
          // Meteor hit lift - create massive explosion and make lift fall!
          console.log(`💥 Meteor hit ${lift.key} lift! MASSIVE EXPLOSION!`);
          
          // Play lifthit sound effect
          playLifthitSound();
          
          // Create massive explosion effect at impact point
          for (let i = 0; i < 50; i++) {
            ...
            gameData.current.bloodSplatters.push({
              ...
              alpha: 1.0,
              color: i % 3 === 0 ? '#ff0000' : (i % 3 === 1 ? '#ff8800' : '#ffff00') // Red, orange, yellow explosion
            });
          }
          ...
          for (let i = 0; i < 30; i++) {
            ...
            gameData.current.bloodSplatters.push({
              x: liftCenterX + (Math.random() - 0.5) * LIFT_WIDTH,
              y: liftCenterY + (Math.random() - 0.5) * LIFT_HEIGHT,
              vx: Math.cos(angle) * speed,
              vy: Math.sin(angle) * speed - 2, // Slight upward bias
              size: 3 + Math.random() * 5,
              alpha: 0.8,
              color: '#666666' // Gray debris
            });
          }
          
          // Set lift to falling state
          liftState.status = 'falling';
          liftState.fallVelocity = 0; // Start with no velocity, will accelerate
  ...
  }, [gameState, playSplatSound, playPopSound, resetLevelAfterDeath, createGoreEffect, getShaftXByIndex, playLifthitSound]);
  ...
  const drawLiftCableAt = (ctx, shaftX, liftY) => {
    ...
    // Determine which lift this is based on shaft position
    const { left: leftShaftX, right: rightShaftX } = getShaftPositions();
    let liftState;
    if (shaftX === leftShaftX) {
      liftState = gameData.current.liftStates.left;
    } else if (shaftX === rightShaftX) {
      liftState = gameData.current.liftStates.right;
    } else {
      liftState = { status: 'normal' }; // fallback
    }
    ...
    if (gameState === GAME_STATES.PLAYING) {
      ...
      if (shouldMove) {
        ...
        // Update center lift position using current dynamic speed and reverse at bounds
        {
          ...
          if (liftState.status === 'falling') {
            // Lift is falling due to meteor impact - accelerate downward
            liftState.fallVelocity += 0.8; // Gravity acceleration
            gameData.current.liftY += liftState.fallVelocity;
            
            // Check if lift hit the ground
            if (gameData.current.liftY + LIFT_HEIGHT >= CANVAS_HEIGHT) {
              gameData.current.liftY = CANVAS_HEIGHT - LIFT_HEIGHT;
              liftState.status = 'crashed';
              liftState.crashedY = gameData.current.liftY;
              liftState.fallVelocity = 0;
              console.log('💥 Center lift crashed to ground!');
              
              // Play liftfall sound effect
              playLiftfallSound();
              
              // Check if player is inside the crashing center lift
              const currentIdx = gameData.current.currentShaftIndex || 0;
              if (currentIdx === 0) { // Player is in center lift
                console.log('💀 Player was inside center lift when it crashed! DEATH!');
                
                // Create massive gore explosion for player death
                createGoreEffect();
                ...
                // Lose a life
                gameData.current.lives--;
                console.log(`Player died in lift crash! Lives remaining: ${gameData.current.lives}`);
              ...
              const liftCenterX = CANVAS_WIDTH / 2;
              for (let i = 0; i < 40; i++) {
                ...
                gameData.current.bloodSplatters.push({
                  ...
                  alpha: 0.9,
                  color: i % 2 === 0 ? '#888888' : '#666666' // Gray dust/debris
                });
              }
              playSplatSound();
            }
          } else if (liftState.status === 'normal') {
            // Normal lift movement
        ...
        {
          const liftState = gameData.current.liftStates.left;
          
          if (liftState.status === 'falling') {
            // Lift is falling due to meteor impact - accelerate downward
            liftState.fallVelocity += 0.8; // Gravity acceleration
            gameData.current.leftLiftY += liftState.fallVelocity;
            
            // Check if lift hit the ground
            if (gameData.current.leftLiftY + LIFT_HEIGHT >= CANVAS_HEIGHT) {
              gameData.current.leftLiftY = CANVAS_HEIGHT - LIFT_HEIGHT;
              liftState.status = 'crashed';
              liftState.crashedY = gameData.current.leftLiftY;
              liftState.fallVelocity = 0;
              console.log('💥 Left lift crashed to ground!');
              
              // Play liftfall sound effect
              playLiftfallSound();
              
              // Check if player is inside the crashing left lift
              const currentIdx = gameData.current.currentShaftIndex || 0;
              if (currentIdx === -1) { // Player is in left lift
                console.log('💀 Player was inside left lift when it crashed! DEATH!');
              ...
              // Create ground impact explosion
              const { left: shaftX } = getShaftPositions();
              const liftCenterX = shaftX + SHAFT_WIDTH / 2;
              for (let i = 0; i < 40; i++) {
                const angle = (Math.PI * 2 * i) / 40 + Math.random() * 0.2;
                const speed = 4 + Math.random() * 6;
                gameData.current.bloodSplatters.push({
        ...
        {
          const liftState = gameData.current.liftStates.right;
          
          if (liftState.status === 'falling') {
            // Lift is falling due to meteor impact - accelerate downward
            liftState.fallVelocity += 0.8; // Gravity acceleration
            gameData.current.rightLiftY += liftState.fallVelocity;
            
            // Check if lift hit the ground
            if (gameData.current.rightLiftY + LIFT_HEIGHT >= CANVAS_HEIGHT) {
              gameData.current.rightLiftY = CANVAS_HEIGHT - LIFT_HEIGHT;
              liftState.status = 'crashed';
              liftState.crashedY = gameData.current.rightLiftY;
              liftState.fallVelocity = 0;
              console.log('💥 Right lift crashed to ground!');
              
              // Play liftfall sound effect
              playLiftfallSound();
              
              // Check if player is inside the crashing right lift
              const currentIdx = gameData.current.currentShaftIndex || 0;
              if (currentIdx === 1) { // Player is in right lift
                console.log('💀 Player was inside right lift when it crashed! DEATH!');
...
useEffect(() => {
    ...
    initializeLifthitSound(); // Initialize meteor hitting lift sound
    initializeLiftfallSound(); // Initialize lift falling/crashing sound
